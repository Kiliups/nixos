...
let
  ...
  playwrightCli = agentSources.playwright-cli or null;
  ...
  agentInstructions = ''
    ...
    ${lib.optionalString (hasPackage "playwright-cli" cfg.packages) "- Use `playwright-cli` when you need Firefox/WebKit, Playwright traces, or annotated video."}
  ...
  agentSkills =
    ...
    // lib.optionalAttrs (hasPackage "playwright-cli" cfg.packages && hasSource playwrightCli) {
  ...
  packageByName = {
    ...
    playwright-cli = playwrightCliBin;
  };
in
{
  options.development = {
    agents = {
      packages = lib.mkOption {
        ...
        description = "Packages installed for enabled AI agents. opencode-desktop is installed only when development.opencode.enable is also enabled. Removing agent-browser or playwright-cli also removes its default skill and instruction.";
        example = [
          "agent-browser"
          "playwright-cli"
        ];
    ...
    opencode.enable = lib.mkEnableOption "OpenCode with shared instructions and skills, programs.mcp.servers integration, the Ponytail plugin, the code-review reviewer agent, and the opc shell alias";
  };
  ...
  config = {
    ...
    xdg.configFile =
      lib.mkIf (config.development.opencode.enable && (hasSource ponytail || hasSource mattPocockSkills))
        {
