...
let
  ...
  agentInstructions = ''
    ...
    - For browser automation and web testing, prefer a capable harness-native browser tool; otherwise use `agent-browser` and keep `agent-browser dashboard start` running so the session stays visible at http://localhost:4848. Use `playwright-cli` instead when you need Firefox/WebKit, Playwright traces, or annotated video. Follow the matching skill in `.agents/skills/` or `.claude/skills/`.
